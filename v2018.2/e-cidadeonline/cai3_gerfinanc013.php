...
$pdf->Text($X + 112,$Y + 12,pg_result($dados,0,"z01_numero"));
$pdf->Text($X + 160,$Y + 12,pg_result($dados,0,"z01_compl"));
...
//if (pg_result($result,0,"k00_matric") == "") {
  //  $TamMatric = db_getMaxSizeField($result,"k00_inscr") * 2;
  //else
  $TamMatric = 10;
//}
$TamNumpar = 4;
...
for ($i = 0; $i < $numrows; $i++) {
  ...
  //echo ' - '.array_search($xnumtot,$tipos);
  ...
  if (($xnumpre != pg_result($result,$i,"k00_numpre")) && ( in_array($xnumtot,$tipos) == true )) {
    $pdf->setx(5);
    $pdf->SetFont('arial','B',6);
    $pdf->Cell(2+$TamNumpar+$TamNumtot+13+13+13+$TamK01_descr+$TamReceit+$TamK02_descr,5,"TOTAL DO NUMPRE ".$xnumpre .($temparcel==true?" - PARCELAMENTO: " . $v07_parcel:(strlen($exercicios) != ""?" - EXERCICIO: " . $exercicios:"")),"T",0,"L",1);
...
$sqlparcel = "select * from termo where v07_numpre = $xnumpre and v07_instit = ".db_getsession('DB_instit') ;
  ...
  if ($linhasparcel>0) {
   ...
   $sqldivida = "select distinct v01_exerc from divida where v01_numpre = $xnumpre and v01_instit = ".db_getsession('DB_instit') ;
     $resultdivida = db_query($sqldivida);
     $exercicios="";
   for ($exerc=0; $exerc < pg_numrows($resultdivida); $exerc++) {
     db_fieldsmemory($resultdivida,0);
...
if (in_array($xnumtot,$tipos) == true ) {
  $pdf->setx(5);
  $pdf->SetFont('arial','B',6);
  //$pdf->Cell(2+$TamNumpar+$TamNumtot+13+13+13+$TamK01_descr+$TamReceit+$TamK02_descr,5,"TOTAL DO NUMPRE ".$xnumpre . (pg_numrows($resultparcel) > 0?" - PARCELAMENTO: " . pg_result($resultparcel,"v07_parcel",0):""),"T",0,"L",1);
  $pdf->Cell(2+$TamNumpar+$TamNumtot+13+13+13+$TamK01_descr+$TamReceit+$TamK02_descr,5,"TOTAL DO NUMPRE ".$xnumpre . ($temparcel==true?" - PARCELAMENTO: " . $v07_parcel:(strlen($exercicios) != ""?" - EXERCICIO: " . $exercicios:"")),"T",0,"L",1);
  $pdf->Cell($TamVlrhis + 6,5,db_formatar($tothisp,'f'),1,0,"R",1);
  $pdf->Cell($TamVlrcor + 6,5,db_formatar($totcorp,'f'),1,0,"R",1);
  $pdf->Cell($TamVlrjuros + 6,5,db_formatar($totjurosp,'f'),1,0,"R",1);
