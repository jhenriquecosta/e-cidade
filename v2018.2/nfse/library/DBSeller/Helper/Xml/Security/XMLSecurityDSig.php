...
class DBSeller_Helper_Xml_Security_XMLSecurityDSig {
  ...
  public function __construct() {
    ...
    $sigdoc->loadXML(DBSeller_Helper_Xml_Security_XMLSecurityDSig::template);
  ...
  public function locateSignature($objDoc) {
    ...
    if ($doc) {
      ...
      $this->sigNode = $nodeset->item($nodeset->length - 1);
      return $this->sigNode;
    }
    return NULL;
  }
  ...
  public function createNewSignNode($name, $value=NULL) {
    ...
    if (! is_null($value)) {
      $node = $doc->createElementNS(DBSeller_Helper_Xml_Security_XMLSecurityDSig::XMLDSIGNS, $this->prefix.':'.$name, $value);
    } else {
      $node = $doc->createElementNS(DBSeller_Helper_Xml_Security_XMLSecurityDSig::XMLDSIGNS, $this->prefix.':'.$name);
    }
    return $node;
  }
  ...
  public function calculateDigest ($digestAlgorithm, $data) {
    ...
    switch ($digestAlgorithm) {
      case DBSeller_Helper_Xml_Security_XMLSecurityDSig::SHA1:
        $alg = 'sha1';
        break;
      case DBSeller_Helper_Xml_Security_XMLSecurityDSig::SHA256:
        $alg = 'sha256';
        break;
      case DBSeller_Helper_Xml_Security_XMLSecurityDSig::SHA384:
        $alg = 'sha384';
        break;
      case DBSeller_Helper_Xml_Security_XMLSecurityDSig::SHA512:
        $alg = 'sha512';
        break;
      case DBSeller_Helper_Xml_Security_XMLSecurityDSig::RIPEMD160:
        $alg = 'ripemd160';
        break;
      default:
    ...
    if ($uri = $refNode->getAttribute("URI")) {
      ...
      if (empty($arUrl['path'])) {
        if ($identifier = $arUrl['fragment']) {
          ...
          $iDlist = '@id=\''.$identifier.'\' or @Id=\''.$identifier.'\'';
          ...
          $aNodesLote  = $refNode->ownerDocument->getElementsByTagName('LoteRps');
          if (empty($dataObject) && $aNodesLote->length  > 0 ) {

             $includeCommentNodes = FALSE;
             $dataObject = $aNodesLote->item(0);
          }
        } else {
  ...
  public function locateKey($node=NULL) {
    ...
    if ($doc = $node->ownerDocument) {
      ...
      if ($algorithm) {
        try {
          $objKey = new DBSeller_Helper_Xml_Security_SecurityKey($algorithm, array('type'=>'public'));
        } catch (Exception $e) {
          return NULL;
        }
        return $objKey;
