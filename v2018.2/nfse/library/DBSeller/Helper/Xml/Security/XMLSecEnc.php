...
class DBSeller_Helper_Xml_Security_XMLSecEnc {
  ...
  private function _resetTemplate(){
    ...
    $this->encdoc->loadXML(DBSeller_Helper_Xml_Security_XMLSecEnc::template);
  }
  ...
  public function getCipherValue() {
    ...
    $xPath->registerNamespace('xmlencr', DBSeller_Helper_Xml_Security_XMLSecEnc::XMLENCNS);
  ...
  public function encryptKey($srcKey, $rawKey, $append=TRUE) {
    ...
    $encKey = $this->encdoc->createElementNS(DBSeller_Helper_Xml_Security_XMLSecEnc::XMLENCNS, 'xenc:EncryptedKey');
    if ($append) {
    ...
    $encMethod = $encKey->appendChild($this->encdoc->createElementNS(DBSeller_Helper_Xml_Security_XMLSecEnc::XMLENCNS, 'xenc:EncryptionMethod'));
    $encMethod->setAttribute('Algorithm', $srcKey->getAlgorith());
    if (! empty($srcKey->name)) {
      $keyInfo = $encKey->appendChild($this->encdoc->createElementNS('http://www.w3.org/2000/09/xmldsig#', 'dsig:KeyInfo'));
      $keyInfo->appendChild($this->encdoc->createElementNS('http://www.w3.org/2000/09/xmldsig#', 'dsig:KeyName', $srcKey->name));
    }
    $cipherData = $encKey->appendChild($this->encdoc->createElementNS(DBSeller_Helper_Xml_Security_XMLSecEnc::XMLENCNS, 'xenc:CipherData'));
    $cipherData->appendChild($this->encdoc->createElementNS(DBSeller_Helper_Xml_Security_XMLSecEnc::XMLENCNS, 'xenc:CipherValue', $strEncKey));
    if (is_array($this->references) && count($this->references) > 0) {
      $refList =  $encKey->appendChild($this->encdoc->createElementNS(DBSeller_Helper_Xml_Security_XMLSecEnc::XMLENCNS, 'xenc:ReferenceList'));
      foreach ($this->references AS $name=>$reference) {
        $refuri = $reference["refuri"];
        $dataRef = $refList->appendChild($this->encdoc->createElementNS(DBSeller_Helper_Xml_Security_XMLSecEnc::XMLENCNS, 'xenc:DataReference'));
  ...
  public function locateEncryptedData($element) {
    ...
    if ($doc) {
      $xpath = new DOMXPath($doc);
      $query = "//*[local-name()='EncryptedData' and namespace-uri()='".DBSeller_Helper_Xml_Security_XMLSecEnc::XMLENCNS."']";
