...
class Doctrine_Auth_Adapter implements Zend_Auth_Adapter_Interface {
    ...
    public function setCredential($credential) {
        $this->_credential = hash('sha1',$credential);
        return $this;
    }
    ...
    protected function _authenticateSetup() {
        ...
        if ($this->_em === null) {
            ...
            $exception = 'A credential column must be supplied for the Doctrine_Auth_Adapter authentication adapter.';
        } elseif ($this->_identity == '') {
            $exception = 'A value for the identity was not provided prior to authentication with Doctrine_Auth_Adapter.';
        } elseif ($this->_credential === null) {
            $exception = 'A credential value was not provided prior to authentication with Doctrine_Auth_Adapter.';
        }
...
WHERE u.' . $this->_identityColumn . ' = ?1';
    ...
    protected function _validateResult($resultIdentities) {
        if (count($resultIdentities) < 1) {
            ...
            if ($resultIdentity->getSenha() != $this->_credential) {    
