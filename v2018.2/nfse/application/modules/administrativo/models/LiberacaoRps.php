 ...
 * @author guilherme
 */
class Administrativo_Model_LiberacaoRps extends E2Tecnologia_Model_Doctrine {

    static protected $entityName = "Administrativo\LiberacaoRps";
    ...
    public static function getRequisicoesPendentes($im = null) {
        ...
        $dql = "SELECT e FROM Administrativo\LiberacaoRps e WHERE
            e.data_liberacao IS NULL";
    ...
    public static function getLiberacao($im) {
        ...
        $query->select('e');
        $query->from('Administrativo\LiberacaoRps', "e");
    ...
    public static function getLiberacoes($im) {
        ...
        $query->where("e.data_liberacao IS NOT NULL");
        $query->andWhere("e.im = ?1");
        $query->setParameter(1, $im);
        $query->orderBy('e.data_liberacao', 'DESC');
    ...
    public function getStatus() {
        if ($this->getDataLiberacao() !== null) {
            if ($this->getDataLimite() != null) {
                ...
                $msg = 'Limite de RPS modificado para: <b>' . $this->getNotaLimite() . '</b>';
            }
        } else {
