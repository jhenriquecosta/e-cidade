...
$oRotulo->label("ed37_i_codigo");
$oRotulo->label("ed37_c_descr");
?>
   ...
   <td valign="top">
    <table border="0" align="left" width="100%">
     <tr>
      ...
      <td>
        <?php  db_input('ed40_i_codigo', 20, $Ied40_i_codigo, true, 'text', 3)?>
      </td>
     </tr>
     <tr>
      <td nowrap title="<?=$Ted40_i_formaavaliacao?>">
        <?php db_ancora($Led40_i_formaavaliacao,"js_pesquisaed40_i_formaavaliacao(true);", $db_opcao1);?>
      </td>
      <td>
        <?php
     ...
     <tr>
      ...
      <td>
       <?php db_input('ed40_c_descr', 40, $Ied40_c_descr, true, 'text', $db_opcao ); ?>
      </td>
     </tr>
     <tr>
     ...
     <tr>
      <td colspan="2">
       ...
       <input name="novo" type="button" id="novo" value="Novo Registro"
        ...
        <input type="button" name="btnImportar" id="btnImportar" value="Importar"
               style="display: <?=$db_opcao == 1 && isModuloEscola() ? '': 'none' ?>;"
               onclick="" >
      </td>
     </tr>
    </table>
...
var sArquivoMensagem = "educacao.escola.db_frmprocedimento.";
...
function js_verificaperc(valor) {
  ...
  if (valor != "") {
    ...
    if (parseFloat(valor) < 0 || parseFloat(valor) > 100) {
      ...
      alert( _M( sArquivoMensagem + 'percentual_invalido') );
...
$('btnImportar').onclick = function() {
    ...
    js_OpenJanelaIframe('','db_iframe_procedimento','func_procedimento.php?lProcedimentosSecretaria=true&funcao_js=parent.importarProcedimento|'+
                  'ed40_i_codigo','Pesquisa de Procedimentos de Avaliação',true
                 );
};

function importarProcedimento( iProcedimento ) {

  db_iframe_procedimento.hide();

  var oParametros           = {};
  oParametros.exec          = 'importarProcedimento';
  oParametros.iProcedimento = iProcedimento;

  new AjaxRequest("edu4_procedimentoavaliacao.RPC.php", oParametros, function(retorno, erro) {
    ...
    location.href="edu1_procedimento002.php?chavepesquisa=" + retorno.iNovoProcedimento;
  }).setMessage( _M( sArquivoMensagem + 'importando_procedimento') ).execute();
}
