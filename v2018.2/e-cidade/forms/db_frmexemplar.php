...
$db_opcao               = 1;
$db_botao1              = false;
$bi23_dataaquisicao_dia = '';
$bi23_dataaquisicao_mes = '';
...
$sSituacaoBotaoQuantidade = ' class="field-size2" ';
if ( $db_opcao != 1) {
  $sSituacaoBotaoQuantidade = ' class="field-size2 readonly" disabled="disabled"';
}
?>
<form name="form1" method="post" action="">
  <fieldset>
    <legend>Exemplares do Acervo</legend>
    <table class="form-container">
      <tr style="display:none;">
        <td nowrap title="<?=$Tbi23_codigo?>">
          <label for="bi23_codigo"><?=$Lbi23_codigo?></label>
        </td>
        <td>
          <?php db_input('bi23_codigo', 10, $Ibi23_codigo, true, 'text', 3); ?>
        </td>
      </tr>
      <tr>
        <td nowrap title="<?=$Tbi23_acervo?>">
          <label for="bi23_acervo"><?php db_ancora(@$Lbi23_acervo,"",3); ?></label>
        </td>
        <td>
          <?php
      ...
      <tr >
        <td nowrap title="<?=$Tbi23_exemplar?>">
          <label for="bi23_exemplar"><?=$Lbi23_exemplar?></label>
        </td>
        <td>
          <?php db_input('bi23_exemplar', 10, $Ibi23_exemplar, true, 'text', 3, ""); ?>
        </td>
      </tr>
      <tr>
        <td nowrap title="<?=$Tbi23_codbarras?>">
          <label for="bi23_codbarras"><?=$Lbi23_codbarras?></label>
        </td>
        <td>
          <?php db_input('bi23_codbarras',20,$Ibi23_codbarras,true,'text',3," onKeyPress='tab(event,5)'"); ?>
          <input name="gerabarras" type="button" id="gerabarras" value="Gerar Codigo" onclick="js_GeraBarras()"
                 <?=($db_opcao!=1?"disabled":"")?> />
        </td>
      </tr>
      <tr>
        <td nowrap title="<?=$Tbi23_dataaquisicao?>">
          <label for="bi23_dataaquisicao"><?=$Lbi23_dataaquisicao?></label>
        </td>
        <td>
          <?php
      ...
      <tr>
        <td nowrap title="<?=$Tbi23_aquisicao?>">
          <label for="bi23_aquisicao">
            <?php
              db_ancora(@$Lbi23_aquisicao,"js_pesquisabi23_aquisicao(true);",$db_opcao);
            ?>
          </label>
        </td>
        <td>
      ...
      <tr>
        ...
        <td>
          <input type="text" name="quantidade" id="quantidade" value="1" <?=$sSituacaoBotaoQuantidade?> />
        </td>
      </tr>
    </table>
  </fieldset>
  ...
  <?php
    ...
    $sCampos .= " bi23_dataaquisicao,           ";
    $sCampos .= " bi23_aquisicao,               ";
    $sCampos .= " case when bi23_situacao = 'S' ";
    $sCampos .= "  then 'ATIVO'                 ";
    $sCampos .= "  else 'INATIVO'               ";
    $sCampos .= " end as bi23_situacao          ";

    $chavepri = array( "bi23_codigo"        => empty($bi23_codigo)        ? '' : $bi23_codigo,
                       "bi23_exemplar"      => empty($bi23_exemplar)      ? '' : $bi23_exemplar,
                       "bi23_acervo"        => empty($bi23_acervo)        ? '' : $bi23_acervo,
                       "bi06_titulo"        => empty($bi06_titulo)        ? '' : $bi06_titulo,
                       "bi23_dataaquisicao" => empty($bi23_dataaquisicao) ? '' : $bi23_dataaquisicao,
                       "bi23_codbarras"     => empty($bi23_codbarras)     ? '' : $bi23_codbarras,
                       "bi04_forma"         => empty($bi04_forma)         ? '' : $bi04_forma,
    ...
    $cliframe_alterar_excluir->campos        = "bi23_codigo, bi23_exemplar, bi23_codbarras, bi04_forma, bi23_dataaquisicao, bi23_situacao ";
